		...
		do
			...
			while j <= 99 do
				...
				do
					local k = 0
					while k < #newInput do
						local opcode = newInput[k + 1]
						local digit1 = newInput[newInput[k + 1 + 1] + 1]
						local digit2 = newInput[data[k + 2 + 1] + 1]
						local savePosition = newInput[k + 3 + 1]
						local _2 = solveOpcode(opcode, digit1, digit2)
						local couldSave = _2[1]
						local result = _2[2]
						local _3 = couldSave and result
						if not couldSave then
							break
						elseif _3 ~= 0 and _3 == _3 and _3 then
							newInput[savePosition + 1] = result
						end
						k += 4
					end
				end
				...
				if newInput[1] == targetValue then
					print(100 * i + j)
					solved = true
				end
				j += 1
